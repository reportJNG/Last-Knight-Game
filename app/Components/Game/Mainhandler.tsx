...
export default function Mainhandler({
  ...
  const updatedplayerxp = {
    ...P,
    stats: {
      ...P.stats,
      level: P.stats.level + xp,
      attack: P.stats.attack + xp,
      speed: P.stats.speed + xp,
    },
  };
  ...
  //adddint usestate = random eatime it lunch easy
  const geusschance = (geuss: number) => {
    return (Math.random() * 11) / geuss;
  };
  const updatedconsoletexting = (
    tester: boolean,
    action: 1 | 2 | 3 | 4 | 5,
  ) => {
    //here is the updatedconsoletexting()
    let tt = "";
    if (tester) {
      consoleupdater(combatText.turn.Player);
      switch (action) {
        case 1:
          tt = combatText.player.parry;
          break;
        case 2:
          tt = combatText.player.dodge;
          break;
        case 3:
          tt = combatText.player.parry;
          break;
        case 4:
          tt = combatText.player.dogefails;
          break;
        case 5:
          tt = combatText.player.parryfails;
      }
    } else {
      consoleupdater(combatText.turn.Boss);
      switch (action) {
        case 1:
          tt = combatText.enemy.parry;
          break;
        case 2:
          tt = combatText.enemy.dodge;
          break;
        case 3:
          tt = combatText.enemy.parry;
          break;
  ...
  const attack = () => {
    //here i need handle health not go under 0 + if he test if he is 0 then boss died and same for player
    if (turn) {
      setBoss((prev) => {
        const newHealth = Math.max(0, prev.stats.health - Player.stats.attack);

        if (newHealth === 0) {
          //here player might win
  ...
  const doge = () => {
    //doge logic is 60% ={if(yes){+30hp}if(no){-10 hp}} /// here we test health not go upper then max health and not below 0
    const chance = geusschance(0.6);
    if (turn) {
      if (chance) {
        //success doge player
      ...
      if (chance) {
        ...
        setBoss((prev) => {
          const newHealth = Math.min(
            BOSSES[level].stats.health,
            prev.stats.health + 30,
          );
          return {
            ...prev,
            stats: {
  ...
  const parry = () => {
    //parry logic :{chance 50% always if (yes){attack+=20} else{health-=40}} //here only try to test if health doesnt go below 0
    const chance = geusschance(0.5);
    if (turn) {
      //here start the chance calc
      if (chance) {
        //parry worked then here player hit strike+20
        setBoss((prev) => {
          const newHealth = Math.max(
            0,
            prev.stats.health - (Player.stats.attack + 20),
          );
        ...
        //parry failed then boss hit player 40
        setPlayer((prev) => {
      ...
      if (chance) {
        //boss was able to parry attack +20

        setPlayer((prev) => {
          const newHealth = Math.max(
            0,
            prev.stats.health - (Boss.stats.attack + 20),
          );
        ...
        //boss failed to parry -40 health
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <div className={styles.midscreentop}>
          <Title
            text={Boss.name}
            winornot={testwinner}
            endedfight={fightindicator}
          />
        </div>
        <div className={styles.righttopscrenn}>
          <Pause
            volume={volume}
            setVolume={setVolume}
            sound={sound}
